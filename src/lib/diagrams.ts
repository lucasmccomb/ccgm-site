...
const hookGate: DiagramSpec = {
  ...
  desc: 'A vertical funnel: a tool call enters a settings.json event registration, reaches one dispatcher process, passes a dashed box listing ten priority-ordered checks, then narrows to a precedence rule that splits into two outcomes.',
  summary:
    'A Bash tool call fans into one dispatcher process, not six. Ten checks run in priority order, and one precedence rule -- not array order across modules -- decides the outcome.',
  steps: [
    ...
    'The matcher "Bash" selects one entry: hooks/pretooluse-bash-dispatch.py, a single process that replaced a six-process chain.',
    'lib/hook_dispatcher.py runs a declarative manifest by priority: git_workflow (10), destructive (20), force_branch_delete (25), smart_rules (30), port_advisory (40), agent_tracking (50), migration_timestamp (60), force_push_main (70), careful (80), pattern (90).',
    ...
    'File edits take a separate arm of the same event: branch-guard.py, auto-approve-file-ops.py, and check-freeze.py are registered on Edit, Write, and NotebookEdit rather than on Bash.',
  ],
  width: 560,
  height: 532,
  nodes: [
    { x: 170, y: 12, w: 220, h: 34, label: 'Bash tool call' },
    {
      ...
      sub: ['~/.claude/settings.json (built by lib/merge.sh)'],
    },
    {
      x: 110,
      y: 128,
      w: 340,
      h: 46,
      label: 'One dispatcher process',
      sub: ['hooks/pretooluse-bash-dispatch.py'],
    },
    { x: 40, y: 194, w: 480, h: 190, kind: 'group' },
    {
      x: 110,
      y: 396,
      w: 340,
    ...
    { x: 50, y: 472, w: 200, h: 46, kind: 'soft', label: 'exit 2 (bypass-proof)', sub: ['hook_utils.hard_block()'] },
    {
      x: 310,
      y: 472,
      w: 200,
  ...
  edges: [
    ...
    { points: [[280, 110], [280, 126]] },
    { points: [[280, 174], [280, 192]] },
    { points: [[280, 384], [280, 394]] },
    { points: [[280, 442], [280, 456], [150, 456], [150, 470]] },
    { points: [[280, 442], [280, 456], [410, 456], [410, 470]] },
  ],
  captions: [
    { x: 60, y: 214, text: 'lib/hook_dispatcher.py runs the manifest in priority order', anchor: 'start' },
    { x: 62, y: 244, text: '10  git_workflow', anchor: 'start' },
    { x: 62, y: 264, text: '20  destructive', anchor: 'start' },
    { x: 62, y: 284, text: '25  force_branch_delete', anchor: 'start' },
    { x: 62, y: 304, text: '30  smart_rules', anchor: 'start' },
    { x: 62, y: 324, text: '40  port_advisory', anchor: 'start' },
    { x: 300, y: 244, text: '50  agent_tracking', anchor: 'start' },
    { x: 300, y: 264, text: '60  migration_timestamp', anchor: 'start' },
    { x: 300, y: 284, text: '70  force_push_main', anchor: 'start' },
    { x: 300, y: 304, text: '80  careful', anchor: 'start' },
    { x: 300, y: 324, text: '90  pattern', anchor: 'start' },
    {
      x: 62,
      y: 360,
  ...
  refs: [
    ...
    {
      path: 'modules/branch-guard/hooks/branch-guard.py',
      role: 'The file-edit arm: an independent PreToolUse gate on Edit, Write, and NotebookEdit.',
    },
    {
      path: 'modules/hooks/hooks/auto-approve-file-ops.py',
      role: 'Also on the file-edit arm, alongside check-freeze.py.',
    },
  ],
};
